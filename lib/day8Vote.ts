 ...
 *   트랙 2  4:00 PM ~ 4:10 PM (10분)
 *
 * 이전에는 각 트랙 발표가 끝나는 자리(12:50, 2:50)에 맞춰 열었는데, 당일 진행이
 * 그 배치와 달라졌습니다. 지금은 두 창이 등을 맞대고 붙어 있습니다. 트랙 1이
 * 닫히는 순간 트랙 2가 열려서, 무대에서 "이제 트랙 2 투표"라고 한 번만 말하면
 * 됩니다. 두 창이 겹치지 않는 것이 중요해요 - 겹치면 트랙 2 발표를 보는 도중에
 * 트랙 1을 찍는 사람이 생기고, 그건 마지막에 본 것이 이기는 판이 됩니다.
 *
 * 진행이 또 밀리면 이 네 값(여기 둘 + 아래 VOTE_CLOSES_AT 둘)만 옮기면 됩니다.
 ...
 * 트랙별 마감 시각 (epoch ms).
 *
 * DECIDED 2026-08-28: 공통 하나였던 마감을 트랙별로 나눴습니다.
 * UPDATED 2026-08-29 아침 (박주형): 트랙 1은 3:00 PM, 트랙 2는 3:40 PM.
 *
 * 트랙 1의 마감이 곧 트랙 2의 오픈입니다(3:00 PM). 한 순간을 두 상수가 공유하는
 * 것은 의도예요. voteWindowState가 마감을 >= 로 보고 오픈을 >= 로 보기 때문에
 * 3:00:00.000에 트랙 1은 닫히고 트랙 2는 열립니다. 사이에 빈 틈이 없습니다.
 *
 * 집계는 3:40 이후입니다.
 *
...
export const VOTE_CLOSES_AT: Record<VoteTrack, number> = {
  ...
  2: new Date("2026-08-29T16:10:00+08:00").getTime(),
};
