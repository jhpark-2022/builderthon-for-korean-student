 ...
 * 12월 이벤트의 이름. **아직 없습니다.**
 *
 * TODO: confirm. 이름이 정해지면 `{ ko, en }`으로 채웁니다. 그때까지 `null`이고,
 * 화면은 `decemberEventLabel()`이 돌려주는 "12월 이벤트"로 그립니다.
 *
 * 채울 때 하지 말 것: "제로백", "2회차", "빌더톤 서울"을 붙이지 마세요.
 * 빌더톤이라는 낱말도 12월에 관해서는 쓰지 않습니다. 형식이 빌더톤일지가 아직
 * 정해지지 않았습니다(매니페스토 IV: 8일이 4일이 되어도, 빌더톤이 다른 무엇이
 * 되어도 된다).
 */
export const DECEMBER_EVENT_NAME: { ko: string; en: string } | null = null;
