...
export const SEOUL_WATERMARK = {
  ...
  // 않고, 히어로가 나가면 revealVh에 걸쳐 떠오르고, #naru부터 어두워집니다. 세로에만
  // 남은 것은 크기(110vw → 90vw)와 밝기(0.8 → 0.5)입니다. 폰에서 밝기 1.0이면 카피
  // 뒤가 시끄럽습니다. dissolveVh는 쓰이지 않습니다(9/18의 "히어로에서 풀린다"가
  // 사라지면서). 키는 되살릴 때를 위해 둡니다.
