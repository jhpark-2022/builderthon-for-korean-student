...
export const SEOUL_WATERMARK = {
  ...
  stages: {
    ...
    // #naru 읽기 판의 덮개 구간(c0 + coverMarginVh)입니다. 같은 날의 startVh 1.2, 1.8은
    // 틈을 옮겼을 뿐이라 지웠습니다. 길이는 덮개 구간이 모자라면 줄어듭니다(minSpanVh까지).
    morph: { spanVh: 1.2 },
    ...
    // DECIDED 2026-09-26 (판 덮개 브리프 2.2): #join 판의 덮개 구간 끝(c1 − coverMarginVh)에서
    // 끝납니다. #join 판 뒤에서 사라지므로 틈 C에는 온전한 싱가포르가, 푸터 쪽에는 빈 하늘이 보입니다.
    dissolve: { spanVh: 0.8 },
    // 판 덮개(2026-09-26, 판 덮개 브리프 2.1~2.3). 형상이 바뀌는 일(떠오름, 건너기, 사라짐)은
    // 읽기 판이 형상 상자를 다 덮는 스크롤 구간 [c0, c1] 안에서만 일어납니다.
    // coverMarginVh: 덮개 구간 양 끝의 여유. minSpanVh: 구간이 모자랄 때 길이를 줄이는 하한.
    coverMarginVh: 0.05,
    minSpanVh: 0.3,
    // 덮개 구간을 못 찾을 때(판이 없는 페이지, 판이 형상보다 좁은 창)만 쓰는 옛 식의 값.
