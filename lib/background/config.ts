...
export const SEOUL_WATERMARK = {
  ...
  stages: {
    ...
    singaporeBright: 0.8,
    // 구간 5: 형상을 거둡니다. spanVh 동안 0으로.
    // 세 곳(싱가포르, 한국, 그 밖)을 말하는 챕터 뒤에 싱가포르 하나만 서 있으면
    // 셋 중 하나를 편드는 그림이 됩니다. 나루 점만 남깁니다.
    // DECIDED 2026-09-26 (판 덮개 브리프 2.2): #join 판 뒤, 형상이 다 사라져 있는 동안 끝납니다.
    // 틈 C에는 온전한 싱가포르가, 푸터 쪽에는 빈 하늘이 보입니다.
    dissolve: { spanVh: 0.8 },
    // 판 덮개. DECIDED 2026-09-26 2차 (사용자: 1728×906에서 바뀌는 것이 보임. "안보이는 부분에
    // 틈을 몇개 더 의도적으로 놓아서 거기는 보이게 해주고, 변화하는게 안보이면 좋겠는데"):
    // 판의 폭은 넓은 창에서 형상보다 좁아(1728에서 판 안쪽 1,136px, 형상 1,166px) 판이 형상을
    // 가리는 것만으로는 바뀌는 모습을 숨길 수 없습니다. 그래서 판이 형상 상자를 **위아래로**
    // 다 덮는 스크롤 구간 [c0, c1]에서는 형상이 사라집니다. 들어갈 때 hideFadeVh에 걸쳐
    // 사라지고 나올 때 hideFadeVh에 걸쳐 돌아옵니다. 형상은 판 사이 틈에서만 보이고, 떠오름,
    // 건너기, 사라짐은 형상이 다 사라져 있는 가운데 구간에서만 일어납니다.
    hideFadeVh: 0.3,
    // 덮개 구간이 이보다 짧은 판에서는 사라지지 않습니다(화면 높이 배). 짧은 판에서 사라졌다
    // 돌아오면 깜빡임으로 보입니다(1440×900의 #gains 판은 덮개가 34px). 그런 판 뒤에서는
    // 형상이 흐리게 비친 채 그대로 서 있고, 바뀌는 일도 배정하지 않습니다.
    minHideVh: 0.4,
    // 덮개 구간을 못 찾을 때(판이 없는 페이지, 판이 형상보다 낮은 창)만 쓰는 옛 식의 값.
    // 챕터 머리 앵커: 건너기는 #naru − morphStartVh, 사라짐은 #join − dissolveStartVh에서 시작.
    // 이 길로 가면 개발 빌드에서 경고합니다.
    coverFallback: { morphStartVh: 1.8, dissolveStartVh: 0.6 },
  },
