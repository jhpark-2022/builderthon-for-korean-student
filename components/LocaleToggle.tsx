...
export default function LocaleToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <button type="button" onClick={toggle} aria-label={t(dict.toggle.aria)}
      className={`inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-xs font-semibold transition hover:border-white/30 ${className}`}>
