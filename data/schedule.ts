...
export const days: DayMeta[] = [
  ...
  },
  {
    ...
    weekday: { ko: "수", en: "Wed" },
    phase: LAB2,
    // 폐지 2026-08-23: 아래의 즉석 인기 투표는 제거됐습니다 (원대로 대표님 지시).
// DECIDED 2026-08-13: Day 5 = 집중 빌드·LAP Time 공유·즉석 인기 투표(빌더스
    // 초이스 가산)·출석(0→100 가산). 해시드·OpenAI 글로벌 해커톤은 행사 후 별도
    // 행사로 이동(협의 중).
    //
    ...
    //
    // (2026-08-23: "거기 붙는 투표"는 폐지됐습니다. 이 날만 열리는 것은 LAP Time
    // 공유와 서로의 빌드에 피드백을 주고받는 자리입니다.)
    //
  ...
  },
  {
    ...
    //
    // runOfShow의 두 발표 슬롯(10:10AM–12:59PM · 1:39PM–3:04PM)과 그것을 설명하는 주석들은
    ...
    runOfShow: [
      ...
      // 다시 만들지 마세요.
      {
        ...
        // 문을 열면서 그날의 순서를 안내하는 하나의 블록이라, 발표에 필요한
        // 10분을 여기서 만들었습니다.
        time: "10AM–10:10AM",
      ...
      // 전부 발표에 줬습니다.
      //
      // DECIDED 2026-08-26 (Day8_발표순서.docx = 발표 순서 정본): 발표 팀 수에
      // 맞춰 시각을 전부 다시 잡았습니다. 이전 배치(슬롯당 120분, 트랙당 12팀
      // 기준)에는 저지먼트 14팀이 들어가지 않았습니다. 여는 순서는 입장 줄과
      // 합쳐 10분을 내줬습니다.
      //
      // UPDATED 2026-08-28: 발표 순서 정본이 21팀으로 확정됐습니다. 트랙 1은 14팀
      // 그대로이고 트랙 2가 8팀에서 7팀이 됐습니다. 트랙 2 슬롯만 12분 줄었고
      // (3:16PM에서 3:04PM으로) 앞뒤 순서는 하나도 움직이지 않았습니다. 발표팀
      // 명단의 정본은 lib/day8Vote.ts의 VOTE_TEAMS이고, 팀 수가 또 바뀌면 그
      // 배열과 여기를 함께 고치세요.
      //
      // 산술은 팀 수에서 나옵니다. 팀당 10분에 팀 사이 전환 2분, 트랙 중간에 5분
      // 휴식이고, 휴식이 들어가는 자리의 전환 2분은 휴식이 대신합니다:
      //   트랙 1 = 14×10 + 12×2 + 5 = 169분 (10:10AM–12:59PM)
      //   트랙 2 =  7×10 +  5×2 + 5 =  85분 (1:39PM–3:04PM)
      // 나머지는 입장·오프닝 10분, 점심 40분, 간담회 60분, 어워드 10분, 클로징 10분이고
      // 3:04–3:20의 16분은 발표에서 간담회로 넘어가는 전환입니다. 합이 4:40PM입니다.
      //
      // 빌더스 초이스 투표는 이 시간표와 나란히 돕니다(정본은 lib/day8Vote.ts).
      // UPDATED 2026-08-29 아침: 두 창을 오후로 옮기고 등을 맞대게 붙였습니다.
      //   트랙 1  2:10PM ~ 3:00PM
      //   트랙 2  3:00PM ~ 3:40PM
      ...
      },
      {
        ...
        note: {
          ko: "7팀 같은 규격. 중간 2:25PM–2:30PM 5분 휴식.",
          en: "7 teams same format. 5 min break at 2:25PM.",
        },
        eventId: "d8-judging",
      },
      {
...
//
// 행사 후 글로벌 해커톤을 여기 적지 마세요.
//
// UPDATE 2026-09-15: 그 해커톤은 무산됐습니다. 비전 섹션의 칸(step 2)도
// 삭제했습니다(data/dictionary.ts의 about.visionSteps 참고). 이 사이트에 그
// 이야기를 하는 자리는 이제 없습니다. 아래 문단은 왜 여기 적지 않았는지에 대한
// 기록으로 남깁니다.
//
// 원래 이유: 협의 중이고 일정과 지원이 모두 미확정이었습니다. 이 사이트에서 그
// 이야기를 하는 자리는 비전 섹션 한 곳뿐이었어요. 헤지가 걸린 문장을 두 곳에서
// 관리하면 한쪽이 반드시 확정형으로 굳습니다.
const HASHED_ORG = {
  name: "Hashed",
...
// DECIDED 2026-08-13: Day 5 = 집중 빌드·LAP Time 공유·즉석 인기 투표(빌더스
  ...
  // 결과 공유회로 재편될 때도 같은 원칙을 썼습니다.
  {
    ...
    description: {
      ...
      // 이어받습니다. "이날 현장에 온 것 자체는 0→100 부문에 반영됩니다"와
