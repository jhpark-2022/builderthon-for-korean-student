...
| 2 | 위상 연속성 | `BackgroundScene`에 개발 빌드 전용 로그: 프레임 간 `ringPhase` 변화가 `maxRate × dt`를 넘는 프레임이 **0건**. 폰 프로파일에서 1,200px 튕김 5회 |
| 3 | 폰 튕김 | 페이지를 열고 60초 기다린 뒤 300px 튕김. 파문이 한 프레임에 한 파장 넘게 움직이지 않음(개발 빌드 로그로 확인, 캡처는 보조) |
