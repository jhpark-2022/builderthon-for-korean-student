# Changelog 2026-09-18 (after: 끝나면 할 일 세 줄)

**2026-09-18 갱신(사용자): "이거를 위한 챕터를 만들지는 말고, 기존 포맷에 몇 줄 더 넣는 느낌."** 아래 1절의
`#after` 챕터·헤더 항목·다리 링크 둘은 그 지시로 걷었습니다. 남은 것은 `#december` 안, 아쉬웠던 넷 바로 아래에
같은 행 형식으로 "끝나면 할 일" 셋(`after.stepsLabel`, `after.steps`). 데이터 키는 전부 그대로 두고 그리지
않습니다. 문서 높이 390 11,514 → 약 11,700, 1440 10,349 → 10,642. 커밋 `d66543f`까지가 챕터 판, 그다음
커밋이 이 판입니다.
